...
## Фичефлаги / переменные окружения (V3 additions)
- `PREVIEW_FIRST=1` — включить режим «предпросмотр сначала»: бот присылает быстрый превью‑результат, затем финал (апскейл) как отдельное сообщение.
- `QUEUE_ENABLED=1` — включает фоновую очередь: апскейл уходит в Redis, worker присылает финал.
...
- Кнопка Requeue для DLQ (перезакидывает задачу обратно в основную очередь).
